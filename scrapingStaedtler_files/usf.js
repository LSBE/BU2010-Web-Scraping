/* USF file - DO NOT MODIFY THIS FILE. THIS FILE IS REGULARLY CHANGED BY USF APP AND **ANY DIRECT CHANGES WILL BE LOST**. Use our in-app customization if you need to update CSS and JS code. Auto modified at: 2/20/2023 5:00:55 PM*/
/* Begin custom theme code */
// define templates for the Canopy-Clean Canvas-4.0.4 theme

var _usfImageWidths;
var _usfGridItemClass = '';

var _usfLabelsTemplate = `
<span v-if="vl = usf.utils.getMetafield(product,'custom','product_promotion_collection_text')" class="product-label product-label--custom-label"><span v-html="vl"></span></span>
<span v-if="_usfGlobalSettings.label_soldout_show && usf.settings.search.showSoldOut && isSoldOut" class="product-label product-label--soldout">
<span v-html="_usfSoldOutText" :class="{'product-label--no-display': product.tags.indexOf('Coming Soon') > -1}"></span>
</span>
<span v-else-if="_usfGlobalSettings.label_sale_show && usf.settings.search.showSale && hasDiscount" class="product-label product-label--sale">
    <span v-html="_usfGlobalSettings.prod_sale_method == 'standard' ? _usfSaleText : _usfGlobalSettings.prod_sale_method == 'money' ? _usfSaveText + ' ' + usfGetDisplayPriceWithSup(displayDiscount) : _usfSaveText + ' ' + salePercent + '%' "></span>
</span>
<span v-else-if="_usfGlobalSettings.label_remain_show && available > 0 && (_usfGlobalSettings.prod_stock_warn_limit_int == 0 || available <= _usfGlobalSettings.prod_stock_warn_limit_int)" class="product-label product-label--low-inventory">
    <span v-html="_usfOnlyLeftText.replace('{{ quantity }}',available)"></span>
</span>
<span v-if="showNew" class="product-label product-label--new-in"><span v-html="_usfNewText"></span></span>`;
var _usfThemeRating = `
<template v-if="ratingMeta != ''">
    <div class="cc-rating" role="img" :data-rating-decimal="rating_decimal" :aria-label="_usfReviewInfo.replace('{{ rating_value }}',ratingMeta.value).replace('{{ rating_max }}',ratingMeta.scale_max)">
        <span aria-hidden="true" class="cc-rating-stars">
            <span class="cc-rating-stars--inactive">
                <svg v-for="i in 5" class="cc-rating-stars__svg" width="13" height="13" viewBox="0 0 13 13" xmlns="http://www.w3.org/2000/svg">
...
<span v-if="ratingCount != '' && parseInt(ratingCount) > 0" class="cc-rating-custom-caption" v-html="ratingCount + ' ' + (parseInt(ratingCount) > 1 ? _usfReviewsLinkText.replace('{{ count }}',ratingCount.toLowerCase()) : _usfReviewLinkText.replace('{{ count }}',ratingCount.toLowerCase()))"></span>`;
...
<div class="usf-sr-container" :class="{'usf-no-facets': noFacets, 'usf-empty': !loader && !hasResults, 'usf-nosearch': !showSearchBox}">
    ...
    <div :data-normheights="view === 'grid' ? '.image' : false" :data-normheights-inner="view==='grid' ? 'img' : false" :class="(view === \'grid\' ? \'product-list product-grid row grid\' : \'list-view-items\') + \' usf-results usf-clear usf-\' + view">
        ...
        <template v-else>
            <template v-if="hasResults">
                <template v-if="view === 'grid'">
                    ...
                    <div v-for="i in _usf_max_cols" class="product-block product-block--flex-spacer flex column" :class="['max-cols-' + _usf_max_cols,'min-cols-' + _usf_min_cols]"></div>
                </template>
                <template v-else>
...
`,
    ...
    <div class="product-block flex column fake-class-test" :class="[_usfGridItemClass,{'sold-out': _usfGlobalSettings.label_soldout_show && usf.settings.search.showSoldOut && isSoldOut,'product-block--no-quickbuy': !_usf_show_quickbuy,'product-block--with-quickbuy': _usf_show_quickbuy && product.variants.length > 1 && !isSoldOut}]" :id="_usfSectionId + '_' + product.id" :product-selector="product.id" :data-usf-pid="product.id">
    <div class="product-block__inner" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave">
        <!-- Wishlist -->
        <usf-plugin name="searchResultsProductWishList" :data="pluginData"></usf-plugin>
        <!-- Labels -->
        <usf-plugin name="searchResultsProductLabel" :data="pluginData"></usf-plugin>
        <div class="image" :class="['image--shape-' + _usfGlobalSettings.prod_thumb_shape,{'image--with-secondary': _usfGlobalSettings.product_grid_hover_image && hoverImage}]">
            <div class="inner">
                    ...
                    <div class="image__primary">
                        <div class="rimage-outer-wrapper" :style="'max-width:' + selectedImage.width + 'px'">
                            <div class="rimage-wrapper lazyload--placeholder" :style="'padding-top:' + 100/_usfGetImageRatio(selectedImage) + '%'">
                                <img class="rimage__image" :data-lazy-src="_usfGetScaledImageUrl(scaledSelectedImageUrl)" loading="lazy" :alt="selectedImage.alt" :width="selectedImage.width" :height="selectedImage.height">
                            </div>
                        </div>
                    </div>
                    <div v-if="_usfGlobalSettings.product_grid_hover_image && hoverImage" class="image__secondary">
                        <div class="rimage-outer-wrapper rimage-background" :data-lazy-bg="_usfGetScaledImageUrl(scaledHoverImageUrl)" :data-parent-fit="_usfGlobalSettings.prod_quick_buy_type == 'hover' && _usfGlobalSettings.prod_quick_buy_pos == 'bottomcorner' ? 'cover' : 'contain'"></div>
                    </div>
                </a>
                <!--quick buy-->
                <template v-if="_usfGlobalSettings.prod_quick_buy_show && _usfGlobalSettings.prod_quick_buy_type == 'hover'">
                    <a class="usf-quick-buy round-button" :class="['pos-' + _usfGlobalSettings.prod_quick_buy_pos]" :href="productUrl" :data-product-id="product.id" @click.prevent.stop="usf.event.raise('preview_show', null, pluginData)">
                        <span :aria-label="_usfQuickBuyText" v-html="_usfSvgSearch"></span>
                    </a>
                    <script type="text/template" class="quickbuy-placeholder-template" :data-product-id="product.id">
        ...
        <!--show weight-->
        <div v-if="_usfGlobalSettings.product_grid_show_weight" class="product-block__weight" v-html="productWeightUnit"></div>
  
  
        <div class="product-block__title-price">
            <a class="title" :href="productUrl">
                <div v-if="_usf_show_vendor && usf.settings.search.showVendor" class="vendor" v-html="product.vendor"></div>
                <span v-html="product.title"></span>
            </a>
            <!-- Product review -->
            <div v-if="_usfGlobalSettings.prod_block_layout != 'beside'" class="themed-product-reviews">
            `+_usfThemeRating+`
            </div>
            <usf-plugin v-if="_usfGlobalSettings.prod_block_layout != 'beside'" name="searchResultsProductReview" :data="pluginData"></usf-plugin>
  
            <div class="price" :class="{'on-sale':hasDiscount}" v-if="!priceLoader">
                <span v-if="priceVaries && !product.selectedVariantId" class="from" v-html="_usfFromText"></span>
                <span class="amount theme-money" v-html="usfGetDisplayPriceWithSup(priceVaries && !product.selectedVariantId ? displayDiscountedPrice : displayDiscountedPrice)"></span>
                <del v-if="hasDiscount"><span class="theme-money" v-html="usfGetDisplayPriceWithSup(priceVaries && !product.selectedVariantId ? displayMinPrice : displayPrice)"></span></del>
                <div class="usf-unit-price" v-html="unitPrice"></div>
            </div>
        </div>
    <!-- Product review -->
    <div v-if="_usfGlobalSettings.prod_block_layout == 'beside'" class="themed-product-reviews themed-product-reviews--bottom">
        `+_usfThemeRating+`
    </div>
    <usf-plugin v-if="_usfGlobalSettings.prod_block_layout == 'beside'" name="searchResultsProductReview" :data="pluginData"></usf-plugin>
    <template v-if="_usf_show_quickbuy">
        <div v-if="product.variants.length > 1" class="product-form">
            <a class="usf-quick-buy btn" :class="{'disabled':isSoldOut}" :href="productUrl" :data-product-id="product.id" v-html="isSoldOut ? _usfUnavailableText : _usfChooseOptionsText" @click.prevent.stop="usf.event.raise('preview_show', null, pluginData)"></a>
            <script type="text/template" class="quickbuy-placeholder-template" :data-product-id="product.id">
                <div class="quickbuy-form placeholder">
              <span class="loading-spinner"></span>
            </div>
        ...
        <template v-else>
            ...
            <form method="post" :action="usf.platform.addToCartUrl" :id="'product_form_' + product.id" accept-charset="UTF-8" class="product-form product-form--mini" enctype="multipart/form-data" :data-ajax-add-to-cart="_usf_enable_ajax ? 'true' : 'false'" :data-open-cart-drawer="window._usf_open_cart_drawer ? 'true' : 'false'" :data-product-id="product.id">
                <input type="hidden" name="form_type" value="product">
                <input type="hidden" name="utf8" value="✓">
                <input type="hidden" name="quantity" value="1" />
                <input type="hidden" name="id" :value="selectedVariantForPrice.id" />
                <button type="submit" name="add" class="btn product-add" :disabled="isSoldOut" aria-live="polite">
                    <span class="button-text" v-html="isSoldOut ? _usfUnavailableText : _usfAddToCartText"></span>
                    <span class="loading-spinner" :aria-label="_usfAddingToCartText"></span>
                    <span class="added-to-cart-check" :aria-label="_usfAddedToCartText"></span>
                </button>
            </form>
            <div class="qty-adjuster-container">
                <div class="qty-adjuster qty-adjuster--ajax" :data-limit="selectedVariantForPrice.available > 0 ? selectedVariantForPrice.available : false">
                    <div class="qty-adjuster__inner">
                        <button class="btn qty-adjuster__down" :aria-label="_usfDecreaseText" v-html="_usf_svg_minus"></button>
                        <input class="qty-adjuster__value" type="text" pattern="[0-9]*" name="quantity" value="" :aria-label="_usfQuantityText">
                        <button class="btn qty-adjuster__up" :aria-label="_usfIncreaseText" v-html="_usf_svg_plus"></button>
                    </div>
                </div>
            </div>
        </template>
...
<ul class="pagination">
    <template v-for="e in elements">
        <li v-if="e.type === 'prev'">
            <a :href="'?page='+(usf.search.page-1)" :title="loc.prevPage" @click="onPrev" class="prev">
        ...
        <li v-else-if="e.type === 'page' && !e.current"><a :href="'?page='+e.page" @click="ev=>onPage(e.page,ev)" :title="usf.utils.format(loc.gotoPage,e.page)">{{e.page}}</a></li>
        <li v-else-if="e.type === 'next'">
            <a :href="'?page='+(usf.search.page+1)" :title="loc.nextPage" @click="onNext" class="next">
                <svg fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                    <title>Right</title>
...
`<a class="usf-sr-product" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave" :href="productUrl" :data-usf-pid="product.id">
    ...
    <div class="usf-info-column">
        ...
        <div class="usf-price-wrapper" :class="{'usf-price--sold-out': isSoldOut}" v-if="!usf.plugins.lastRenderResult && !priceLoader" :data-variant-id="product.selectedVariantId">
        ...
        <div class="usf-preview__body">
            ...
            <div class="usf-preview__content-right usf-scrollbar">
                <div class="usf-preview__content-summary">
                    ...
                    <!--Prices -->
                    <div class="usf-preview__price-wrapper" :class="{'price--sold-out': isSoldOut}" v-if="!priceLoader">
...
`<span class="usf-is-product usf-clear" @click="onItemClick">
    ...
    <div class="usf-pull-left">
        ...
        <!-- Prices -->
        <div class="usf-price-wrapper" v-if="!priceLoader">
            <span class="usf-price" :class="{ 'usf-has-discount': hasDiscount }" v-html="displayPrice"></span>
...
usf.event.add('init', function () {    
    ...
    _usfGridItemClass = 'layout-align-' + _usfGlobalSettings.prod_block_layout + ' max-cols-' + _usf_max_cols + ' min-cols-' + _usf_min_cols + ' product-block--gutter-' + _usfGlobalSettings.prod_thumb_gutter + ' product-block--gap-' + _usfGlobalSettings.prod_thumb_gap + ' product-block--border-' + _usfGlobalSettings.prod_thumb_show_border;
    ...
    var CustomGridItem = {
        ...
        created() {
            var t = this;
            if(_usfGlobalSettings.product_grid_show_weight){
                fetch(`/products/` + t.product.urlName + '?view=usf-weight&variant=' + this.selectedVariantForPrice.id, {
                    credentials: 'same-origin',
                    method: 'GET'
                }).then(function (response) {
                    return response.text() 
                }).then(rs => { 
                    t.productWeightUnit = rs;
                }); 
            }
            fetch(`/products/` + t.product.urlName + '?view=usf-unit-price&variant=' + this.selectedVariantForPrice.id, {
                credentials: 'same-origin',
                method: 'GET'
            }).then(function (response) {
                return response.text() 
        ...
        computed:{
            showNew(){
                ...
                var showNew = false;
                if(_usfGlobalSettings.label_soldout_show && usf.settings.search.showSoldOut && this.isSoldOut)
                    showSoldOut = true;
                if(_usfGlobalSettings.label_sale_show && usf.settings.search.showSale && this.hasDiscount)
                    showSale = true;
                if(_usfGlobalSettings.label_remain_show && this.available > 0 && (_usfGlobalSettings.prod_stock_warn_limit_int == 0 || this.available <= _usfGlobalSettings.prod_stock_warn_limit_int))
                    showRemain = true;
                if(!showSoldOut && !showSale && !showRemain && _usfGlobalSettings.label_new_in_show){
                    if(_usfGlobalSettings.prod_new_method == 'date'){
                        showNew = usfIsNew(this.product.date);
                    }else if(_usfGlobalSettings.prod_new_method == 'tag'){
                        showNew = this.product.tags.includes('New');
                    }else{
                        for(let i = 0; i < this.product.collections.length;i++){
                            var colId = this.product.collections[i];
                            var conHandle = _usfCollectionById[colId];
                            if(conHandle && conHandle == 'new'){
                                showNew = true;
                                break;
                            }
                        }
            ...
            ratingCount(){
                return usf.utils.getMetafield(this.product, 'reviews', 'rating_count');
            }
        },
    }
    ...
    usf.event.add(['sr_updated', 'sr_viewChanged', 'rerender'], function () {
        setTimeout(function () {
            if(window.theme && theme.Sections){
                
                if(theme.FilterManager){
                    theme.Sections.register("collection-template", theme.FilterManager, {deferredLoad: !1});
                    theme.Sections.register('search-template', theme.FilterManager, { deferredLoad: false });
                }
                if(theme.CollectionTemplateSection){
...
function _usfOnAddToCartSuccess(rs, formSelector) {
    window.theme && theme.updateCartSummaries && theme.updateCartSummaries(window._usf_open_cart_drawer)
}
...
function _usfSetDefaultSettings(){
    ...
    window._usfGlobalSettings = window._usfGlobalSettings || {
        prod_block_layout: "above align-center",
        prod_thumb_gutter: "0",
        prod_thumb_gap: "10",
        prod_thumb_show_border: true,
        label_soldout_show: false,
        prod_thumb_shape: "natural",
        product_grid_hover_image: true,
        prod_quick_buy_type: "in-page",
        prod_quick_buy_pos: "middle",
        prod_quick_buy_show: true,
        prod_quick_buy_type: "in-page",
        label_sale_show: true,
        prod_sale_method: "money",
        label_remain_show: false,
        prod_stock_warn_limit_int: 5,
        label_new_in_show: false,
        prod_new_in_limit_int:14,
        prod_new_method: "date",
        product_grid_show_weight: true,
        prod_block_layout: "above align-center",
        superscript_decimals: true,
      };
      window._usf_max_cols = window._usf_max_cols || 3;
      window._usf_min_cols = window._usf_min_cols || "2";
      window._usfSectionId = window._usfSectionId || "template--__main";
      window._usfSvgSearch = window._usfSvgSearch || `<svg width="19px" height="21px" viewBox="0 0 19 21" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n  <g stroke="currentColor" stroke-width="0.5">\n    <g transform="translate(0.000000, 0.472222)">\n      <path d="M14.3977778,14.0103889 L19,19.0422222 L17.8135556,20.0555556 L13.224,15.0385 C11.8019062,16.0671405 10.0908414,16.619514 8.33572222,16.6165556 C3.73244444,16.6165556 0,12.8967778 0,8.30722222 C0,3.71766667 3.73244444,0 8.33572222,0 C12.939,0 16.6714444,3.71977778 16.6714444,8.30722222 C16.6739657,10.4296993 15.859848,12.4717967 14.3977778,14.0103889 Z M8.33572222,15.0585556 C12.0766111,15.0585556 15.1081667,12.0365 15.1081667,8.30827778 C15.1081667,4.58005556 12.0766111,1.558 8.33572222,1.558 C4.59483333,1.558 1.56327778,4.58005556 1.56327778,8.30827778 C1.56327778,12.0365 4.59483333,15.0585556 8.33572222,15.0585556 Z"></path>\n    </g>\n  </g>\n</svg>`;
      window._usfQuickBuyText = window._usfQuickBuyText || usf.settings.translation.quicView;
      ...
      window.usf_money_format = window.usf_money_format || "${{amount_with_comma_separator}}";
      window._usfChooseOptionsText = window._usfChooseOptionsText || usf.settings.translation.chooseOptions;
      window._usfUnavailableText = window._usfUnavailableText || "Unavailable";
      window._usfAddToCartText = window._usfAddToCartText || usf.settings.translation.addToCart;
      window._usfAddingToCartText = window._usfAddingToCartText || "Adding";
      window._usfAddedToCartText = window._usfAddedToCartText || "Added to cart";
      window._usfDecreaseText = window._usfDecreaseText || "Decrease";
      
      window._usf_svg_minus = window._usf_svg_minus || `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-minus"><line x1="5" y1="12" x2="19" y2="12"></line></svg>`;
      window._usf_svg_plus = window._usf_svg_plus || `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>`;
      window._usfQuantityText = window._usfQuantityText || usf.settings.translation.quantity;
      window._usfIncreaseText = window._usfIncreaseText || "Increase";
      window._usfReviewsLinkText = window._usfReviewsLinkText || "Reviews";
      window._usfReviewLinkText = window._usfReviewLinkText || "Review";
      window._usfTotalReviewsText = window._usfTotalReviewsText || "total reviews";
...
var usfIsNew = function (day) {
    ...
    return diffDays < _usfGlobalSettings.prod_new_in_limit_int 
}

function usfGetDisplayPriceWithSup(price) {
    if (!(usf_money_format.includes('money') || usf_money_format.includes('.')) && window._usfGlobalSettings.superscript_decimals) {
        if (usf_money_format.includes('{{amount}}') || usf_money_format.includes('{{ amount }}') || usf_money_format.includes('{{amount_with_comma_separator}}') || usf_money_format.includes('{{ amount_with_comma_separator }}')) {
...
usf.event.add('init', function () { 
    ...
    var NewSearchResultsItemBase = {
        ...
        created() { 
            var t = this;
            var root = Shopify.routes.root != '/' ? Shopify.routes.root : '';
            var url = Shopify.routes.root + `products/` + this.product.urlName + '?view=usf-data'
            fetch(url, {
               credentials: 'same-origin',
               method: 'GET'
            }).then(function (response) {
